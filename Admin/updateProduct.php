            ...
            <div class="card">
                ...
                <div class="card-body">
                                ...
                                <div class="row">
                                    <form method="post" enctype="multipart/form-data">
                                        ...
                                        <div class="col-sm-12">
                                            <div class="form-group">
                                                <label>Ảnh</label>
                                                <input name="txtpic" type="file" class="form-control" accept=".jpg, .jpeg, .png" />
                                                <small class="form-text text-muted">Ảnh hiện tại: <img src="upload/<?php echo $se_pro['image']; ?>" alt="Current Image" width="50" height="50" /> (Nếu không chọn ảnh mới, ảnh cũ sẽ được giữ nguyên)</small>
                                            </div>
                                        </div>
                                        <div class="col-sm-12">
                                            <div class="form-group">
                                                ...
                                                <small class="form-text text-muted">Chọn ảnh mới để thêm vào thư viện (ảnh cũ sẽ được giữ nếu không chọn ảnh mới)</small>
                                            </div>
                                        </div>
                                        <div class="col-sm-12">
                                            <div class="form-group">
                    ...
                    if (isset($_POST['txtsub'])) {
                        ...
                        $select_pro_id = $get_data->select_product_id($id_pro);
                        $current_product = mysqli_fetch_assoc($select_pro_id);
                        $current_image = $current_product['image'];

                        // Kiểm tra xem có ảnh mới được tải lên không
                        $mainImage = !empty($_FILES['txtpic']['name']) ? $_FILES['txtpic']['name'] : $current_image;
                        ...
                        $update = $get_data->update_product(
                            ...
                            $_POST['txtname'],
                            $_POST['txtnum'],
                            $mainImage, // Sử dụng ảnh mới hoặc ảnh cũ
                            $_POST['txtselect'],
                            $_POST['txtdate'],
                            $_POST['txtprice'],
                            $price_sale,
                        ...
                        if ($update) {
                            ...
                            // Xử lý thư viện ảnh (chỉ xóa ảnh cũ nếu có ảnh mới được tải lên)
                            $uploadDirectory = 'upload/';
                            if (!empty($_FILES['albumImages']['name'][0])) {
                                // Xóa ảnh cũ trong thư viện nếu có ảnh mới
