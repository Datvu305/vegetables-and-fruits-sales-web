...
.qr-code {
    text-align: center;
    margin: 0 0 20px 0; /* Reduced top margin to move QR code upward */
}
...
<head>
    ...
    <title>Ogani | Thanh toán ngân hàng</title>
...
<body>
    ...
    <div class="container">
        <!-- Dải chữ chạy được đặt ngay đầu section, trước tiêu đề -->
                ...
                <div class="container">
                        ...
                        <div class="col-lg-12">
                            <div class="contact__form__title">
                                <h2>Quét mã QR dưới đây để thanh toán qua Vietcombank</h2>
                                <p></p>
                            </div>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="qr-code">
                                <img id="qrCode" src="" alt="VietQR Code">
                            </div>
                            <form id="paymentForm" method="post">
                                ...
                                <input type="hidden" name="optradio" id="optradio" value="Chuyển khoản qua ngân hàng">
                                <input type="hidden" name="total" id="total">
                               <center> <button type="submit" name="txtsub" class="site-btn">Đã thanh toán</button></center>
                            </form>
                        </div>
                    </div>
                </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (!checkoutData) {
            alert('Dữ liệu thanh toán không tồn tại. Vui lòng quay lại trang thanh toán.');
            window.location.href = 'checkout.php';
            return;
        }

        // Populate hidden form fields
        document.getElementById('txtname').value = checkoutData.txtname || '';
        document.getElementById('txtPhone').value = checkoutData.txtPhone || '';
        document.getElementById('diachi').value = checkoutData.diachi || '';
        document.getElementById('optradio').value = checkoutData.optradio || 'Chuyển khoản qua ngân hàng';

        // Calculate total from cart (server-side)
        <?php
        $subTotal = 0;
        if (isset($_SESSION['user'])) {
        ...
        const username = '<?php echo isset($_SESSION["user"]) ? $_SESSION["user"] : "KhachHang"; ?>';
        const qrUrl = `https://vietqr.co/api/generate/vcb/0357130945/VIETQR.CO/${total}/Chuyentien${username}?style=1&logo=1&isMask=1&bg=61`;
        document.getElementById('qrCode').src = qrUrl;
    });
    </script>

    <?php
    if (isset($_POST['txtsub']) && $_POST['optradio'] === 'Chuyển khoản qua ngân hàng') {
        $name = $_POST['txtname'];
        ...
        if ($insert) {
            if (isset($_SESSION['user'])) {
                ...
                if ($insert_order) {
                    ...
                    echo "<script>sessionStorage.removeItem('checkoutData'); alert('Đặt hàng thành công'); window.location='shop-grid.php';</script>";
                }
            } else {
